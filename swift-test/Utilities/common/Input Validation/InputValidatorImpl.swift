...
class InputValidatorImpl: IInputValidator {
    ...
    func validateRequiredField(_ text: String) -> ValidationMessage {
        if text.isEmpty {
            return ValidationMessage(message: Constants.CANNOT_BE_EMPTY, validationType: .requiredField)
        }
        
        return ValidationMessage(isValid: true, message: "", validationType: .requiredField)
    }
    
    func validateAlphaNumerics(_ text: String) -> ValidationMessage {
        let regex = "[0-9A-Za-z]{2,}"
        let predicate = NSPredicate(format: "SELF MATCHES %@", regex)
        
        if text.isEmpty {
            return ValidationMessage(message: Constants.CANNOT_BE_EMPTY, validationType: .address)
        } else if !predicate.evaluate(with: text.trimmingCharacters(in: .whitespacesAndNewlines)) {
            return ValidationMessage(message: "Must be at least 2 characters", validationType: .address)
        }
        
        return ValidationMessage(isValid: true, message: "", validationType: .address)
    }
    ...
    func validate(_ value: String, for type: ValidationType) -> ValidationMessage {
        ...
        case .alphaNumeric:
            return validateAlphaNumerics(value)
        }
    }
