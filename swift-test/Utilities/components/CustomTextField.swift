...
class CustomTextField: BaseView {
    fileprivate let titleLabel = UILabel(text: "Title Label", font: .avenirRegular(size: 14), numberOfLines: 0, color: .primaryTextColor, alignment: .left)
    fileprivate let errorLabel = UILabel(text: "Error Label", font: .avenirRegular(size: 13), numberOfLines: 0, color: .systemRed, alignment: .left)
    let textField = TextField()
    fileprivate let passwordIconImageView = UIImageView(image: UIImage(named: "eye_open_icon"), tintColor: .primaryTextColor, size: 22)
    ...
    convenience init(title: String, placeholder: String = "", isPassword: Bool = false, height: CGFloat? = 80, validationType: ValidationType? = nil, keyboardType: UIKeyboardType = .alphabet, maxLength: Int? = nil) {
        ...
        with(textField) {
            ...
            $0.attributedPlaceholder = NSAttributedString(string: placeholder, attributes: [.font: UIFont.avenirRegular(size: 14), .foregroundColor: UIColor.aPlaceholderText])
            $0.font = .avenirRegular(size: 15)
            $0.delegate = self
    ...
    func togglePasswordVisibility() {
        if !passwordVisible && text.isNotEmpty {
            ...
            passwordIconImageView.image = UIImage(named: "eye_closed_icon")
        } else {
            textField.isSecureTextEntry = true
            passwordVisible = false
            passwordIconImageView.image = UIImage(named: "eye_open_icon")
        }
    }
    ...
    fileprivate func addDropDownChevron() {
        ...
        dropButton.setImage(UIImage(named: "arrow_down_icon"), for: .normal)
